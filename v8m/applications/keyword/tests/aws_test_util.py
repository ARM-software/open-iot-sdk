...
class Flags:
    def __init__(self, build_dir, credentials_dir):
        ...
        self.OTA_BINARY = 'keyword_signed_update.bin'
        self.OTA_BINARY_PATH = os.path.join(
            self.BUILD_DIR, 'examples', 'keyword', self.OTA_BINARY)
        ...
        self.OTA_UPDATE_FILES = [
            {
                ...
                'codeSigning': {
                    'customCodeSigning': {
                        'signature': {
                            'inlineDocument': bytearray(read_whole_file(os.path.join(self.BUILD_DIR, 'examples/keyword/update-signature.txt')).strip(), 'utf-8')
                        },
                        'certificateChain': {
                            'certificateName': '0'
                        },
...
def cleanup_aws_resources(flags: Flags):
    if flags.update:
        try:
            iot.delete_ota_update(otaUpdateId=flags.update, forceDeleteAWSJob=True, deleteStream=True)
