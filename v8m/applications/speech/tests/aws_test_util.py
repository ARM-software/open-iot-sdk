...
class Flags:
    def __init__(self, build_dir, credentials_dir):
        ...
        self.OTA_BINARY = 'speech_signed_update.bin'
        self.OTA_BINARY_PATH = os.path.join(
            self.BUILD_DIR, 'examples', 'speech', self.OTA_BINARY)
        ...
        self.OTA_UPDATE_FILES = [
            {
                ...
                'codeSigning': {
                    'customCodeSigning': {
                        'signature': {
                            'inlineDocument': bytearray(read_whole_file(os.path.join(self.BUILD_DIR, 'examples/speech/update-signature.txt')).strip(), 'utf-8')
                        },
                        'certificateChain': {
                            'certificateName': '0'
                        },
