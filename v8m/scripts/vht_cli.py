...
def get_avh_imgid(region):
    ...
    try:
        out_im = subprocess.check_output(
            cmd_im, stderr=subprocess.STDOUT, shell=True
        ).decode("utf-8")
    except subprocess.CalledProcessError as e:
        logging.error(
            red
            + "Could not get Arm Virtual Hardware Instance ID for region {}.".format(
                region
            )
            + nc
        )
...
def start_avh(profile, key, region, initfile):
    ...
    try:
        ...
        if errtype == "VPCIdNotSpecified":
            logging.info(
                orange
                + "Running the start command failed: default VPC not found. Creating VPC..."
                + nc
            )
...
def status_avh(profile, region, printv):
    ...
    cmd = 'aws ec2 describe-instances --query "Reservations[*].Instances[*].[State.Name, ImageId, PublicIpAddress, InstanceId]" --profile {} --region {} --output json'.format(
        profile, region
    )
    ...
    for reservation in range(len(outd)):
        for instance in range(len(outd[reservation])):
            if outd[reservation][instance][1] == ImgId:
                ...
                arr_inst.append(
                    [
                        outd[reservation][instance][3],
                        outd[reservation][instance][2],
                        outd[reservation][instance][0],
                    ]
                )
    ...
    if printv == True:
        ...
        for i in arr_inst:
            if i[2] == "running":
                print(
                    "| {}\t| {}\t| ".format(i[0], i[1]) + green + "RUNNING" + nc + "\t|"
                )
...
def stop_avh(profile, region):
    ...
    if n == 0:
        ...
        instanceid = input(
            orange
            + "The instance will be terminated and all data will be lost. "
            + nc
            + "Please enter the instance ID to stop it (leave empty to cancel): "
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="AVH command-line interface for AWS. Starts (launches) a single AVH AMI instance and stops (terminates) it."
    )
    parser.add_argument(
        "-p",
        dest="profile",
        ...
        action="store",
        type=str,
        help=("Name of the profile configuration (default is 'default')"),
        default="default",
    )
    parser.add_argument(
        "-k",
        ...
        action="store",
        type=str,
        help=("Name of the AWS key. If not specified, a key will be created"),
    )
    parser.add_argument(
        "-r",
        dest="region",
        ...
        action="store",
        type=str,
        help=("Name of the AWS region (default is 'us-east-1')"),
        default="us-east-1",
    )
    parser.add_argument(
        "-i",
        ...
        help=("Location of cloud-init file for instance configuration"),
    )
    parser.add_argument(
        "command",
        ...
        action="store",
        type=str,
        help=("Start, status or stop command"),
    )
    ...
    try:
        awscli_version_f = (
            subprocess.check_output("aws --version", shell=True)
            .decode("utf-8")
            .split()[0]
            .split("/")[1]
        )
    ...
    awscli_version_m = float(
        awscli_version_f.split(".")[0] + "." + awscli_version_f.split(".")[1]
    )
    if awscli_version_m < 2.2:
        logging.warning(
            orange
            + "AWS CLI version is: {}. Minimum required is 2.2".format(awscli_version_m)
            + nc
        )
    ...
    try:
        awscli_version_f = subprocess.check_output(
            "aws configure list --profile {}".format(args.profile), shell=True
        )
    except subprocess.CalledProcessError:
        ...
        if args.key == None:
            ...
            if (
                os.path.isfile(
                    os.path.join(
                        os.path.expanduser("~"),
                        ".ssh/avh_user_{}.pem".format(args.region),
                    )
                )
                == False
            ):
                logging.info(
                    orange
                    + "No key specified and avh_user_{} key not found, creating 'avh_user_{}' key...".format(
                ...
                try:
                    ...
                    logging.error(
                        red
                        + "Saving key in {}.ssh failed.".format(
                ...
                logging.info(
                    orange
                    + "Key has been saved as {}.ssh".format(
                        os.path.expanduser("~") + os.path.sep
                    )
                    + os.path.sep
                    + "avh_user_{}.pem.".format(args.region)
                    + nc
                )
            else:
                logging.info(
                    orange
                    + "Using 'avh_user_{}' key in {}.ssh ...".format(
                        args.region, os.path.expanduser("~") + os.path.sep
                    )
                    + nc
                )
