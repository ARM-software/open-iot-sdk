...
THING_NAME_PREFIX = "ota-ci-test-thing-"
OTA_NAME_PREFIX = 'ota-test-update-id-'
OTA_S3_BUCKET_PREFIX = 'ota-ci-test-bucket-'
OTA_BINARY = 'keyword_signed_update.bin'
    ...
    for ota_update in response['otaUpdates']:
        ...
        if update_id.startswith(OTA_NAME_PREFIX):
            try:
                ...
                if ota_response['otaUpdateInfo']['otaUpdateStatus'] == 'DELETE_IN_PROGRESS':
                    print(f'Skip ota job {update_id}, delete in progress')
                else:
                    iot.delete_ota_update(otaUpdateId=update_id, forceDeleteAWSJob=True, deleteStream=True)
...
def process_things(nextToken: str) -> str:
    ...
    for thing in response['things']:
        ...
        if thing_name.startswith(THING_NAME_PREFIX):
            try:
                iot.detach_thing_principal(thingName=thing_name, principal=f'arn:aws:iot:{AWS_REGION}:{AWS_ACCOUNT}:cert/{OTA_CERT_ID}')
